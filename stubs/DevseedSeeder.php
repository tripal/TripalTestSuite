...
//  protected $interpro_file = ['file_remote' => 'https://raw.githubusercontent.com/statonlab/tripal_dev_seed/master/Fexcel_mini/ips/polypeptide_mini.fasta.xml'];
  ...
  private
  function load_landmarks($run_args, $file) {
  ...
  private
  function load_mRNA_FASTA($run_args, $file) {
    module_load_include('inc', 'tripal_chado', 'includes/TripalImporter/FASTAImporter');
    $importer = new \FASTAImporter();
    $importer->create($run_args, $file);
    $importer->prepareFiles();
  ...
  private
  function load_polypeptide_FASTA($run_args, $file) {
    module_load_include('inc', 'tripal_chado', 'includes/TripalImporter/FASTAImporter');
  ...
  private
  function load_interpro_annotations($run_args, $file) {
  ...
  private
  function load_GFF($run_args, $file) {
  ...
  private
  function load_blast_annotations($run_args, $file) {
  ...
  private
  function load_biomaterials($run_args, $file) {
  ...
  private
  function load_expression($run_args, $file) {
  ...
  private
  function fetch_chado_record($table, $fields, $factory_array) {
